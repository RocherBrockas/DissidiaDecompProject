...
def getModuleInfoAddrFromLoadCommands():
    ...
    # DEBUG temporaire - affiche les composants du load command
    print("=== Load Command Components ===")
    for i in range(loadcmd.getNumComponents()):
        comp = loadcmd.getComponent(i)
        print("  [{}] {} = {} (len={})".format(i, comp.getFieldName(), comp.getValue(), comp.getLength()))

    # Lire en 32-bit avec getInt au lieu de getLong
    load_offset = loadcmd.getComponent(1).getInt(0) & 0xFFFFFFFF
    load_paddr  = loadcmd.getComponent(3).getInt(0) & 0xFFFFFFFF

    print("load_offset: 0x{:x}".format(load_offset))
    print("load_paddr:  0x{:x}".format(load_paddr))

    # Kernel mode PRX : masquer le bit haut
    load_paddr &= 0x7FFFFFFF

    # Pour les OVL : load_paddr EST l'adresse virtuelle absolue
    # sceModuleInfo est a l'offset (load_paddr - load_offset) dans le segment
    # mais le segment est charge a image_base, donc :
    image_base = currentProgram.getImageBase().getAddressableWordOffset()
    
    raw = (load_paddr - load_offset) & 0xFFFFFFFF
    print("raw (paddr - offset): 0x{:x}".format(raw))
    print("image_base:           0x{:x}".format(image_base))

    # Si raw < image_base, c'est un offset relatif -> ajouter image_base
    # Si raw >= image_base, c'est deja une adresse absolue
    if raw < image_base:
        final_addr = (raw + image_base) & 0xFFFFFFFF
    else:
        final_addr = raw

    print("final sceModuleInfo addr: 0x{:x}".format(final_addr))
